...
module.exports.ingredient_create_post = [
  body("ingredient_name", "Ingredient Name required")
    .trim()
    .isLength({ min: 1 })
    .escape(),
  body("ingredient_in_stock", "Ingredient must be in or out of stock")
    .trim()
    .isIn(["on", "off"])
    .escape(),
...
module.exports.ingredient_update_post = function (req, res, next) {
  res.send(
    "Not Implemented Yet: Update Ingredient ID:" + JSON.stringify(req.params.id)
  );
};
